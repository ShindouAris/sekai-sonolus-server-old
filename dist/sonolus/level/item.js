...
import { getMusicBgmPath, getMusicChartPath, getMusicCoverPath, getMusicPreviewPath, } from '../../clients/asset/music.js';
...
export const updateLevelItems = (repository) => {
    const levels = [];
    for (const musicDifficulty of Object.values(repository.musicDifficulties)) {
        ...
        if (!music)
            continue;
        const chart = asset(musicDifficulty.server, getMusicChartPath(music.id, musicDifficulty.musicDifficulty));
        if (!repository.whitelist.has(chart.url))
            continue;
        const musicVocals = Object.values(repository.musicVocals).filter(({ musicId }) => musicId === music.id);
        ...
        for (const musicVocal of musicVocals) {
            ...
            if (!repository.whitelist.has(bgm.url))
                continue;
            const name = `${config.sonolus.prefix}-${music.id}-${musicVocal.id}-${musicDifficulty.musicDifficulty}`;
            const preview = asset(musicVocal.server, getMusicPreviewPath(musicVocal.assetbundleName));
            const characterIds = musicVocal.characters.map((character) => `${character.characterType}_${character.characterId}`);
            ...
            levels.push({
                ...
                artists: characters.length
                    ? join(characterSeparator, characters.map((character) => character.title))
                    : musicVocalTypeTitle,
    ...
    sonolus.level.items = levels.sort((a, b) => b.meta.publishedAt - a.meta.publishedAt ||
