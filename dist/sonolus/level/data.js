    ...
    sonolus.router.get('/sonolus/levels/:name/data', (req, res) => {
        ...
        res.sendFile(path, async (error) => {
            ...
            if (!error)
                return;
            try {
                const buffer = await getLevelData(item.meta.musicId, item.meta.difficulty, item.meta.fillerSec, item.meta.server);
                res.send(buffer);
...
export const getLevelDataUrl = (name) => `/sonolus/levels/${name}/data?${version}`;
const getLevelData = async (id, difficulty, fillerSec, server) => {
