...
export const installBackgroundList = () => {
    sonolus.background.listHandler = ({ search: { type, options }, page, options: { spoilers }, }) => {
        ...
        if (type === 'others')
            return {
                ...paginateItems(filterBackgrounds(sonolus.background.items.filter(noMeta), options.keywords), page),
                searches: backgroundSearches,
            };
        const items = filterBackgrounds(filteredBackgrounds.filter(({ meta }) => meta &&
            options.characters[meta.characterId] &&
            options.rarities[meta.rarity] &&
            options.attributes[meta.attribute] &&
            options.images[meta.image]), options.keywords);
        return {
