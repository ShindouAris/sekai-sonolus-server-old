...
export const installPlaylistList = () => {
    sonolus.playlist.listHandler = ({ search: { type, options }, page, options: { spoilers } }) => {
        ...
        const items = filterPlaylists(filteredPlaylists.filter(({ meta }) => (!meta.characterIds.size ||
            [...meta.characterIds].some((characterId) => options.artists[characterId])) &&
            (!meta.musicVocalTypes.size ||
                [...meta.musicVocalTypes].some((musicVocalType) => options.categories[musicVocalType]))), options.keywords);
        return {
