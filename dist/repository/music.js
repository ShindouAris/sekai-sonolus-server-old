...
export const getMusics = (data) => mapValues(match(merge(data.musics, 'id', [
    'title',
    'pronunciation',
    'lyricist',
    'composer',
    'arranger',
]), data.musicTitles, (music, locale, musicTitle) => {
    music.title[locale] ??= musicTitle;
}), (_, music) => {
    const pronunciations = new Set(Object.values(music.title));
    const romajis = new Set([...pronunciations].map((pronunciation) => toRomaji(pronunciation)));
